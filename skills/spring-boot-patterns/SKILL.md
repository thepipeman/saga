...
- **Business context** — the problem this type or method solves, the rule it enforces, the invariant it protects, who consumes it and why that shapes the contract.
- **Technical context and the decision behind it** — why it's built this way, the tradeoff taken, and any non-obvious constraint (ordering, idempotency, concurrency, external-call boundaries, compensating logic).

**Never reference how the code was produced.** No mention of design docs, specs, instruction or requirement files, implementation notes, tickets-as-process, review phases, workflow steps, AI, agents, models, or prompts — and no `.claude/` paths. A future reader has none of that context, and a comment pointing at an artifact they can't find (or that has since changed) is worse than no comment. Describe the *code's* reasoning, not the process that generated it.

```java
// Wrong — points at process artifacts the reader has no access to
/**
 * Implements the refund flow described in the approved design doc
 * (.claude/design-drafts/refunds-design.md), per the AI implementation pass.
 * Test scope was limited to unit tests as requested.
 */
 ...
 *
 * <p>Refunds are capped at the settled amount because the payment provider
 * rejects over-refunds asynchronously, hours later, with no correlation id —
 * so the cap is enforced here rather than relied on downstream.
 *
 * <p>The ledger write commits before the provider is called: the call is not
 * transactional, and a failure after commit is reconciled by
 * {@link RefundReconciliationJob} rather than rolled back.
 *
 * @param paymentId settled payment to refund
...
Skip Javadoc where it would only restate the signature — trivial getters, self-evident DTO records, straightforward delegation. An accurate short comment on the three methods that carry real business rules beats a docblock on every method. Same rule for inline comments: explain *why*, never narrate *what* the line does.
