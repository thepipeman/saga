...
**Default to none.** Write Javadoc only where the code can't speak for itself — one of:

- **Business context** — the rule enforced, the invariant protected, the problem solved.
- **Technical decision** — why it's built this way, or a non-obvious ordering, idempotency, or concurrency constraint.

Skip it on trivial getters, self-evident records, plain delegation, and test methods whose name already says it. One accurate line on the two methods carrying real business rules beats a docblock on every method.

**One or two lines.** A summary sentence, plus a single short `<p>` only for a genuine hack or workaround a reader would otherwise "fix". Longer than that is an essay — cut it. Inline comments explain *why*, never narrate *what*.

**Describe the code as it stands, not the change that produced it.** "now", "previously", "unlike before", "used to", "refactored to" — banned; history lives in git. Where a prior state is load-bearing (a workaround for an external bug), state the constraint, not the story.

**Never reference how the code was produced** — design docs, specs, instruction files, implementation notes, review phases, workflow steps, AI, agents, models, prompts, `.claude/` paths. The reader has none of that context, and a pointer to an artifact they can't find is worse than no comment.
 ...
 * {@link AbstractIntegrationTest}, unlike before when it hand-rolled its own — see
 * the approved design doc (.claude/design-drafts/refunds-design.md). The rows persisted
 * by a real initiate-then-verify flow are cleaned up here, in FK-safe order, or they'd
 * linger and trip other tests' table-wide cleanup, e.g. ...
 */

// Right — the constraint, stated once
/** Deletes persisted rows in FK-safe order; the shared context leaks them into other tests otherwise. */

// Right — business context plus the decision and its reason
/**
 * Issues a refund against a settled payment, capped at the settled amount.
 *
 * <p>The provider rejects over-refunds asynchronously with no correlation id, so the
 * cap cannot be enforced downstream.
 *
