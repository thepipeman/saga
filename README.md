...
This plugin is 100% AI-coded — every command, agent, hook, and skill file was
written by Claude. It is not, however, unsupervised: the workflow it
implements, the gates it enforces, and the conventions baked into each skill
come from the author's own experience building this kind of system plus
research into how skills and subagents actually behave in practice. The
steering is human; the typing is not.

<b>The AI is not liable for what it produces, and neither is this plugin</b>. Code
generated by an LLM — including code generated *by* this workflow — is a
draft that still needs a human who understands it to review, test, and take
responsibility for it before it ships. `/mark-reviewed` is a gate this plugin
enforces mechanically; it cannot verify that the review was actually
thorough. That part is still on you.

> "Pushing broken, untested garbage to PR is laziness dressed up as work.
> Your reviewers are engineers, not babysitters — their time is worth more
> than cleaning up your mess. If your PR gets rejected for something a
> five-minute test would have caught, that's not a process failure. That's a
> you failure. Own it, or find a different profession."
>
> — a fictional developer, in the spirit of Linus Torvalds
...
/implement                         # phase 2: confirms the recommended model (if not sonnet) and asks test scope (none/unit/unit+integration — not asked when the design is test-only), then implements, writes .claude/implementation-notes/proj-123-notes.md
# ... you review the diff ...
