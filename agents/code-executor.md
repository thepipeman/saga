...
12. **Default to no Javadoc; where it earns its place, keep it to a line or two.** Write it only for **business context** (the rule enforced, the invariant protected) or a **technical decision** (why it's built this way, a non-obvious ordering/idempotency/concurrency constraint). Expand past two lines only for a genuine hack a reader would otherwise undo — no essays. Skip it entirely where it would restate the signature: trivial getters, self-evident records, plain delegation, test methods whose name already says it. Describe the code as it stands, never the change that produced it ("now", "previously", "unlike before" are banned — history lives in git). Never mention the design doc, the spec or instruction file, implementation notes, `.claude/` paths, test-scope decisions, review phases, AI, agents, models, or prompts. Inline comments explain *why*, never narrate *what*. Worked examples are in the `spring-boot-patterns` skill. Process context belongs in your report (rule 13) and the implementation notes (rule 14) — never in the source.
